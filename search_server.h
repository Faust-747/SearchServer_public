...
private://========================================================================
    ...
    std::map<int, std::map<std::string, double>> document_to_word_freqs_; //новый контейнер для GetWordFrequencies - заполняем в AddDocument (не меняя старое)
    ...
    DocumentPredicate document_predicate) const 
{
    ...
    LOG_DURATION_STREAM("Результаты поиска по запросу: "s + raw_query, std::cout);
